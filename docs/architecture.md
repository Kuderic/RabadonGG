...
3. Backend fetches matchup data for each candidate in the role pool:
...
5. Data is cached to disk (`backend/data/{patch}/{lane}/`) with a 1-day TTL and pre-loaded into process memory at server startup.
...
- **No database** — all persistent state is file-cached data. Stateless per request beyond that.
- **No auth** — MVP is a local dev tool.
- **No LLM calls yet** — build/rune synthesis is a planned feature, not implemented.
- **Data Dragon** is used only for champion name → image URL mapping. Champion IDs match CIDs exactly.

...
async def draft_overview(request: DraftOverviewRequest) -> DraftOverviewResponse:
    ...
    # Ensure slug↔ID map is loaded before spawning coroutines. patch may be "30"
    # (days window) which is not a valid DDragon version, so always use the real patch.
    await _ensure_champion_map(await _get_patch())
    ...
    # Pre-fetch role pools only when open slot recommendations are requested
    if request.compute_open:
        open_roles = {
            s.role
            for s in (request.ally + request.enemy)
    ...
    async def build_slot(slot: "DraftSlot", own_locked: list, opp_locked: list) -> DraftSlotResult:
        ...
        if not request.compute_open:
            return DraftSlotResult(role=slot.role, locked=False, candidates=[])

        pool = pools.get(slot.role, [])
        cand_recs = await asyncio.gather(*[score_one(c, slot.role, own_locked, opp_locked) for c in pool])
        scored = sorted(
            [r for r in cand_recs if r is not None],
            key=lambda r: r.rating,
            reverse=True,
        )
        return DraftSlotResult(role=slot.role, locked=False, candidates=scored[:10])
